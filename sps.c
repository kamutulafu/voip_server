...
int h264_decode_sps(BYTE* buf,
  ...
  *fps = 0;
  ...
  // buf[4], buf[5]); de_emulation_prevention(buf, &nLen); printf("%02x %02x %02x
  ...
  if (nal_unit_type == 7) {
    ...
    if (pic_order_cnt_type == 0) {
      ...
      int* offset_for_ref_frame =
          malloc(sizeof(int) * num_ref_frames_in_pic_order_cnt_cycle);
    ...
    *height = (pic_height_in_map_units_minus1 + 1) * 16;
    ...
    if (timing_info_present_flag) {
      printf("H.264 SPS: -> video size %dx%d, %d fps, profile(%d) %s\n", *width,
             *height, *fps, profile_idc, profile_str);
    } else {
